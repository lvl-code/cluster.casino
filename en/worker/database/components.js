...
export async function assignComponentToPage(db, data) {
  return await db.prepare(`
    INSERT INTO page_components (page_type, page_slug, component_id, position, enabled)
    VALUES (?, ?, ?, ?, ?)
  `).bind(
    data.page_type,
    data.page_slug,
...
export async function getPageComponents(db, pageType, pageSlug) {
  const result = await db.prepare(`
    SELECT pc.*, c.name, c.slug, c.type, c.title, c.content, c.settings_json, c.status
    FROM page_components pc
    JOIN components c ON c.id = pc.component_id
    WHERE pc.page_type = ? AND pc.page_slug = ? AND pc.enabled = 1
...
export async function getAllPageAssignments(db, pageType = null, pageSlug = null) {
  if (pageType && pageSlug) {
    const result = await db.prepare(`
      ...
      FROM page_components pc
      JOIN components c ON c.id = pc.component_id
      WHERE pc.page_type = ? AND pc.page_slug = ?
      ORDER BY pc.position ASC
    `).bind(pageType, pageSlug).all();
    return result.results || [];
  }
  const result = await db.prepare(`
    ...
    FROM page_components pc
    JOIN components c ON c.id = pc.component_id
    ORDER BY pc.page_type, pc.page_slug, pc.position ASC
  `).all();
  return result.results || [];
}
